...
{   
    ...
    if (abs(deltaX) < BALL_RADIUS + this->size.x / 2)
    {
        if (ball->buffType == 1) 
        {
            ...
            ball->isDeleted = true;
            for (list<Ball*>::iterator ballIt2 = model->balls.begin(); ballIt2 != model->balls.end(); ballIt2++)
            { 
            model->addEvent({model->currentTime + newBall->timeToCollision(*ballIt2), newBall, *ballIt2});
            }
            for (list<Brick*>::iterator brickIt = model->bricks.begin(); brickIt != model->bricks.end(); brickIt++)
            {
            model->addEvent({model->currentTime + (*brickIt)->timeToCollision(newBall), *brickIt, newBall});
            }
            for (list<Wall*>::iterator wallIt = model->walls.begin(); wallIt != model->walls.end(); wallIt++)
            {
                model->addEvent({model->currentTime + (*wallIt)->timeToCollision(newBall), *wallIt, newBall});
            }
            model->addEvent({model->currentTime + model->paddle.timeToCollision(newBall), &(model->paddle), newBall});
            return;
        }
