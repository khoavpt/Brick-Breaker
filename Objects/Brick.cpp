...
{
    sf::Vector2f ballPos = ball->body.getPosition();
    sf::Vector2f brickPos = body.getPosition();

    ball->velocity.x = -ball->velocity.x;
    sf::Vector2f newBallPos = ballPos + ball->velocity * 0.01f;
    if (abs(newBallPos.x - brickPos.x) < BRICK_WIDTH / 2.0f + BALL_RADIUS && abs(newBallPos.y - brickPos.y) < BRICK_HEIGHT/ 2.0f + BALL_RADIUS)
    {
