...
Ball::Ball(sf::Vector2f position, sf::Vector2f velocity, int buffType)
{
    ...
    else if(buffType == 2) body.setFillColor(sf::Color::Yellow);
...
float Ball::timeToCollision(Ball* other)
{
    ...
    float squareDist = (dotProduct(delta_r, delta_r) - totalRadius * totalRadius);
    float delta = dotProduct_vr * dotProduct_vr - dotProduct(delta_v, delta_v) * squareDist;
    if (dotProduct(delta_v, delta_r) >= 0 || delta < 0 || squareDist < 0) return INFINITY;
